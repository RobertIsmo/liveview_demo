defmodule LiveviewDemo.Games do
  ...
  def increment_counter(%Counter{} = counter, by) when is_integer(by) do
    {1, [%Counter{count: count}]} =
      from(c in Counter, where: c.id == ^counter.id, select: [:count])
    |> Repo.update_all(inc: [count: by])

    put_in(counter.count, count)
  end
end
