defmodule LiveviewDemoWeb.CounterLive.FormComponent do
  ...
  def update(%{counter: counter} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Games.change_counter(counter))
     end)}
  end

  @impl true
  def handle_event("validate", %{"counter" => counter_params}, socket) do
    changeset = Games.change_counter(socket.assigns.counter, counter_params)
  ...
  defp save_counter(socket, :edit, counter_params) do
    case Games.update_counter(socket.assigns.counter, counter_params) do
      {:ok, counter} ->
  ...
  defp save_counter(socket, :new, counter_params) do
    case Games.create_counter(counter_params) do
      {:ok, counter} ->
        notify_parent({:saved, counter})
